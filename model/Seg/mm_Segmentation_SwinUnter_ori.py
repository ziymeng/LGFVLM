...
class SwinEncoder(nn.Module):
    ...
    def forward(self, x_in):
        ...
        _, deep_feat = self.DVF_branch(x_in)
        ...
        enc3 = self.feat_fusion(deep_feat,enc3)
...
class FeatureFusionConcat(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.controller = UnetOutBlock(spatial_dims=3, in_channels=18, out_channels=out_channels)
...
class Vision_Language_Merge_Branch(nn.Module):
    def __init__(self, out_channels=3, text_prompt=True, encoding="word_embedding", text_embedding_name="CLIP_embedding", res_depth=50) -> None:  ## change to 3
    ...
    def forward(self, x1, x2):
        # [1, 768, 4,4,4]
        x1_enc, x1_dec_out = self.encoder(x1, x2)
        ...
        if self.text_prompt:
            ...
            # fusion_feature = torch.cat([x1_seg_out, x2_seg_out], 1)
        else:
...
class MM_SwinUnter_Segmentation(nn.Module):
    def __init__(self, n_class = 7, text_prompt = True, VL_CrossAttention = True, fusion_module="Cross_Attention", text_prompt_name="CLIP_embedding", text_encoding="word_embedding", res_depth=50) -> None:
        ...
        self.VLM_branch = Vision_Language_Merge_Branch(n_class, text_prompt, encoding=text_encoding, text_embedding_name=text_prompt_name, res_depth = res_depth)
        ...
        if fusion_module=="DoubleAttention":
            ...
            self.multimoal_fusion_module = MultimodalCrossAttention(embed_dim=n_class, num_heads=3)
        elif fusion_module == "Cross_Attention":
            self.multimoal_fusion_module = CrossAttention(n_class)
    ...
    def forward(self, x1, x2):
        seg_x1, seg_x2 = self.VLM_branch(x1, x2)
        seg_feature_fusion= self.multimoal_fusion_module(seg_x1, seg_x2)
        return seg_feature_fusion, seg_x1, seg_x2

...
from model.Seg.resnet import Deep_Vision_Feature_Model
...
def window_reverse(windows, window_size, dims):
    ...
    return x
class MoE_Gating_Network(nn.Module):
    def __init__(self, dim, hidden_dim, mlp_ratio,num_experts):
        super(MoE_Gating_Network, self).__init__()
        ...
        self.mlp = Mlp(hidden_size=dim, mlp_dim=mlp_hidden_dim, act="GELU", dropout_rate=drop, dropout_mode="swin")
        self.softmax = nn.Softmax(dim=-1)
        self.TopK=heapq.nlargest(k=1, dim=dim) #
    ...
    def forward(self, x):
        ...
        Exp_x = [expert(x) for expert in self.experts]
        Exp_x = self.mlp(torch.stack(Exp_x, dim=0))
        # compute the router's output
        R_w = self.Weight(x)
        Rou_x = self.softmax(self.TopK(R_w))
        # combine the experts' outputs
        output = (Rou_x.unsqueeze(0) * Exp_x).sum(dim=0)
        print(output)
        return output
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        num_experts = 3 # Eye, Brain, Liver of experts
        self.mlp = Mlp(hidden_size=dim, mlp_dim=mlp_hidden_dim, act=act_layer, dropout_rate=drop, dropout_mode="swin")
        self.MoE_layer = MoE_Gating_Network(dim=dim, hidden_dim=mlp_hidden_dim, mlp_ratio=mlp_ratio, drop=drop, num_experts=num_experts)
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.use_checkpoint:
            ...
            x = x + self.forward_part2(x) + self.MoE_layer(x)
        else:
            x = x + self.forward_part2(x) + self.MoE_layer(x)
        return x
...
class AttentionGateFusion(nn.Module):
    def __init__(self, in_channels_A=2048, in_channels_B=192, out_channels=192):
        super().__init__()
...
class SwinEncoder(nn.Module):
    ...
    def __init__(
            ...
            spatial_dims: int = 3,
            res_depth: int = 50,
            fusion_module: Union[Tuple, str]= "Multimoal_Cross_Attention",
        ...
        if fusion_module=="DoubleAttention":
            self.multimoal_fusion_module = DoubleAttention(n_class)
        elif fusion_module=="Attention_Fusion":
            self.multimoal_fusion_module = AttentionFusion(n_class)
        elif fusion_module=="Multimoal_Cross_Attention":
            self.multimoal_fusion_module = MultimodalCrossAttention(embed_dim=2048, num_heads=4)
        elif fusion_module == "Cross_Attention":
            self.multimoal_fusion_module = CrossAttention(768)
        elif fusion_module=="Concat_Fusion":
            self.multimoal_fusion_module = FeatureFusionConcat(in_channels=n_class, out_channels=n_class)
        elif fusion_module=="SingleAttention":
            self.multimoal_fusion_module = SingleAttention(n_class)
    ...
    def forward(self, x1, x2):
        # for modality x1
        x1_hidden_states_out = self.swinViT(x1, self.normalize)
        _, x1_deep_feat = self.DVF_branch(x1)
        x1_enc0 = self.encoder1(x1)
        x1_enc1 = self.encoder2(x1_hidden_states_out[0])
        x1_enc2 = self.encoder3(x1_hidden_states_out[1])
        x1_enc3 = self.encoder4(x1_hidden_states_out[2])
        x1_dec4 = self.encoder10(x1_hidden_states_out[4])
        x1_enc3 = self.feat_fusion(x1_deep_feat,x1_enc3) #

        # for modality x2
        x2_hidden_states_out = self.swinViT(x2, self.normalize)
        _, x2_deep_feat = self.DVF_branch(x2)
        x2_enc0 = self.encoder1(x2)
        x2_enc1 = self.encoder2(x2_hidden_states_out[0])
        x2_enc2 = self.encoder3(x2_hidden_states_out[1])
        x2_enc3 = self.encoder4(x2_hidden_states_out[2])
        x2_dec4 = self.encoder10(x2_hidden_states_out[4])
        x2_enc3 = self.feat_fusion(x2_deep_feat,x2_enc3) #

        fuse_dec4 = self.multimoal_fusion_module(x1_dec4,x2_dec4)

        x1_dec3 = self.decoder5(fuse_dec4, x1_hidden_states_out[3])
        x1_dec2 = self.decoder4(x1_dec3, x1_enc3)
        x1_dec1 = self.decoder3(x1_dec2, x1_enc2)
        x1_dec0 = self.decoder2(x1_dec1, x1_enc1)
        x1_seg_out = self.decoder1(x1_dec0, x1_enc0)

        x2_dec3 = self.decoder5(fuse_dec4, x2_hidden_states_out[3])
        x2_dec2 = self.decoder4(x2_dec3, x2_enc3)
        x2_dec1 = self.decoder3(x2_dec2, x2_enc2)
        x2_dec0 = self.decoder2(x2_dec1, x2_enc1)
        x2_seg_out = self.decoder1(x2_dec0, x2_enc0)

        return x1_enc3, x2_enc3, x1_seg_out, x2_seg_out
...
class FeatureFusionConcat(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.controller = UnetOutBlock(spatial_dims=3, in_channels=in_channels*2, out_channels=out_channels)
...
class Vision_Language_Merge_Branch(nn.Module):
    def __init__(self, out_channels=3, text_prompt=True, fusion_module="", encoding="word_embedding", text_embedding_name="CLIP_embedding", res_depth=50) -> None:  ## change to 3
        ...
        self.GAP = nn.Sequential(
            ...
            nn.Conv3d(192, 256, kernel_size=1, stride=1, padding=0),
            # nn.Flatten()
        )
        ...
        # self.controller = nn.Linear(384, 256)
        self.controller = nn.Conv3d(384, sum(weight_nums+bias_nums), kernel_size=1, stride=1, padding=0)
        self.seg_head = UnetOutBlock(spatial_dims=3, in_channels=48, out_channels=out_channels)
        ...
        self.multimoal_fusion_module = FeatureFusionConcat(in_channels=self.class_num , out_channels=self.class_num )
    ...
    def forward(self, x1, x2):
        # [1, 768, 4,4,4]
        x1_enc, x2_enc, x1_dec_out, x2_dec_out = self.encoder(x1, x2)
        # x2_enc, x2_dec_out = self.encoder(x2)
        if self.text_prompt:
            ...
            x1_feat = self.GAP(x1_enc)
            x2_feat = self.GAP(x2_enc)
            b = x1_feat.shape[0]
            x1_logits_array = []
            x2_logits_array = []
            for i in range(b):
                ...
                x1_head_inputs = self.precls_conv(x1_dec_out[i].unsqueeze(0))
                ...
                x2_head_inputs = self.precls_conv(x2_dec_out[i].unsqueeze(0))
            ...
            x1_seg_out = self.seg_head(x1_dec_out)
            x2_seg_out = self.seg_head(x2_dec_out)
            fusion_feature = self.multimoal_fusion_module(x1_seg_out,x2_seg_out)

        return fusion_feature, x1_seg_out, x2_seg_out
...
class CrossAttention(nn.Module):
    ...
    def forward(self, A, B):
        ...
        B_, C, D, H, W = A.shape
        Q = self.query(A).view(B_, C, -1)  # [B, C, N]
        K = self.key(B).view(B_, C, -1)    # [B, C, N]
        V = self.value(B).view(B_, C, -1)  # [B, C, N]

        attn = torch.bmm(Q.transpose(1, 2), K) / (C ** 0.5)  # [B, N, N]
        attn = self.softmax(attn)  # [B, N, N]

        out = torch.bmm(attn, V.transpose(1, 2))  # [B, N, C]
        out = out.transpose(1, 2).view(B_, C, D, H, W)
        return out
...
class DoubleAttention(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.attn_A = SingleAttention(channels)
        self.attn_B = SingleAttention(channels)

    def forward(self, A, B):
        A = self.attn_A(A)
        B = self.attn_B(B)
        return (A + B) / 2  # 或 torch.cat([A, B], dim=1)
...
class MM_SwinUnter_Segmentation(nn.Module):
    def __init__(self, n_class = 7, text_prompt = True, VL_CrossAttention = True, use_moe=True, fusion_module="", text_prompt_name="CLIP_embedding", text_encoding="word_embedding", res_depth=50) -> None:
        super(MM_SwinUnter_Segmentation, self).__init__()
        self.text_prompt = text_prompt
        ...
        self.VLM_branch = Vision_Language_Merge_Branch(n_class, text_prompt, fusion_module=fusion_module, encoding=text_encoding, text_embedding_name=text_prompt_name, res_depth = res_depth)
        self.seg_head = UnetOutBlock(spatial_dims=3, in_channels=64, out_channels=n_class)
    ...
    def forward(self, x1, x2):
        fuse_out, seg_x1, seg_x2 = self.VLM_branch(x1, x2)
        return fuse_out, seg_x1, seg_x2
    ...
    if __name__ == "__main__":
        ...
        model = MM_SwinUnter_Segmentation(n_class=4)
        # load pretrain model

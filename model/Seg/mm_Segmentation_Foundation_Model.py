...
class MM_Foundation_Model_Segmentation(nn.Module):
    def __init__(self, use_text_prompt, img_size, n_class=16):
        super().__init__()
    ...
    def forward(self, crop_img,MRI_img):
        crop_img_F_ds, crop_Skips = self.image_encoder(crop_img)
        MRI_img_F_ds, MRI_Skips = self.image_encoder(MRI_img)

        crop_img_F_mia = self.MIA_module(crop_img_F_ds)
        ...
        if self.text_prompt:
            ...
            x1_feat = self.GAP(crop_img_F_ds)
            x2_feat = self.GAP(MRI_img_F_ds)
            b = x1_feat.shape[0]
            x1_logits_array = []
            x2_logits_array = []
            for i in range(b):
                ...
                x1_head_inputs = self.precls_conv(crop_Skips[i].unsqueeze(0))
                ...
                x2_head_inputs = self.precls_conv(MRI_Skips[i].unsqueeze(0))
            ...
            out_fuse = self.fusion_layer(crop_img_F_mia, MRI_img_F_mia)

        out_fuse = self.fusion_layer(crop_img_F_mia, MRI_img_F_mia)
        crop_F_z = torch.cat([out_fuse, crop_img_F_mia], dim=1)
        MRI_F_z = torch.cat([out_fuse, MRI_img_F_mia], dim=1)
        crop_F_z = self.conv3d_convert(crop_F_z)
        MRI_F_z = self.conv3d_convert(MRI_F_z)

        crop_seg_out = self.decoder(crop_F_z, crop_Skips)
        MRI_seg_out = self.decoder(MRI_F_z, MRI_Skips)

        return crop_img_F_ds, MRI_img_F_ds, crop_seg_out, MRI_seg_out

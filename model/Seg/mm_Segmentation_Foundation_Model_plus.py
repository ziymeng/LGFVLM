...
class AttentionGateFusion(nn.Module):
    def __init__(self, in_channels_A=2048, in_channels_B=192, out_channels=512):
        ...
        self.gate = nn.Sequential(
            nn.Conv3d(1024, out_channels, kernel_size=1),
            nn.ReLU(),
...
class SwinEncoder(nn.Module):
    ...
    def __init__(
        ...
        if fusion_module=="DoubleAttention":
            ...
            self.multimoal_fusion_module = MultimodalCrossAttention(embed_dim=768, num_heads=3)
...
class UNet3D(nn.Module):
    def __init__(self, out_channels=2, res_depth=50, fusion_module="Cross_Attention", act='relu'):
    ...
    def forward(self, x1, x2):
        ...
        _, x1_deep_feat = self.DVF_branch(x1)
        x1_enc = self.feat_fusion(x1_deep_feat,self.out512_x1) #
        ...
        _, x2_deep_feat = self.DVF_branch(x2)
        x2_enc = self.feat_fusion(x2_deep_feat,self.out512_x2) #


        # fuse_dec4 = self.multimoal_fusion_module(x1_enc,x2_dec4)

        self.out_up_256_x1 = self.up_tr256(x1_enc,self.skip_out256_x1)
        ...
        self.out_up_256_x2 = self.up_tr256(x2_enc,self.skip_out256_x2)
        ...
        return x1_enc, x2_enc, self.out_up_64_x1, self.out_up_64_x2
...
class Vision_Language_Merge_Branch(nn.Module):
    def __init__(self, out_channels=3, text_prompt=True, fusion_module="Cross_Attention", encoding="word_embedding", text_embedding_name="CLIP_embedding", res_depth=50) -> None:  ## change to 3
        super(Vision_Language_Merge_Branch, self).__init__()
        self.text_prompt = text_prompt
        self.encoding = encoding
        self.text_embedding_name = text_embedding_name
        self.encoder = UNet3D(out_channels= out_channels,res_depth=res_depth,fusion_module="Cross_Attention")
        ...
        self.seg_head = OutputTransition(64, out_channels)
...
class MM_Foundation_Model_Segmentation(nn.Module):
    def __init__(self, n_class = 7, text_prompt = True, VL_CrossAttention = True, fusion_module="Cross_Attention", text_prompt_name="CLIP_embedding", text_encoding="word_embedding", res_depth=50) -> None:
        super(MM_Foundation_Model_Segmentation, self).__init__()
        self.text_prompt = text_prompt
        self.text_encoding = text_encoding
        self.res_depth = res_depth
        self.text_prompt_name = text_prompt_name
        self.VL_CrossAttention = VL_CrossAttention
        self.VLM_branch = Vision_Language_Merge_Branch(n_class, text_prompt, fusion_module="Cross_Attention", encoding=text_encoding, text_embedding_name=text_prompt_name, res_depth = res_depth)

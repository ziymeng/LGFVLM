...
def get_loader(args,train_modality='crop'):
    data_dir = args.data_dir
    datalist_json = os.path.join(data_dir, args.json_list)
    crop_train_transform = transforms.Compose(
        [
            ...
            transforms.Spacingd(
                keys=["img_MRI", "mask_MRI"], pixdim=(args.space_x, args.space_y, args.space_z), mode=("bilinear", "nearest")
            ),
            transforms.ScaleIntensityRanged(
                keys=["img_MRI"], a_min=args.a_min, a_max=args.a_max, b_min=args.b_min, b_max=args.b_max, clip=True
            ),
    ...
    crop_val_transform = transforms.Compose(
        [
            transforms.LoadImaged(keys=["img_MRI", "mask_MRI"],image_only=False),
            transforms.EnsureChannelFirstd(keys=["img_MRI", "mask_MRI"], channel_dim="no_channel"),
            transforms.Orientationd(keys=["img_MRI", "mask_MRI"], axcodes="RAS"),
    ...
    nocrop_train_transform = transforms.Compose(
        [
            transforms.LoadImaged(keys=["img_MRI", "mask_MRI"],image_only=False),
            transforms.EnsureChannelFirstd(keys=["img_MRI", "mask_MRI"], channel_dim="no_channel"),
            transforms.Orientationd(keys=["img_MRI", "mask_MRI"], axcodes="RAS"),
    ...
    nocrop_val_transform = transforms.Compose(
        [
            transforms.LoadImaged(keys=["img_MRI", "mask_MRI"],image_only=False),
            transforms.EnsureChannelFirstd(keys=["img_MRI", "mask_MRI"], channel_dim="no_channel"),
            transforms.Orientationd(keys=["img_MRI", "mask_MRI"], axcodes="RAS"),
    ...
    crop_test_transform = transforms.Compose(
        [
            transforms.LoadImaged(keys=["img_MRI", "mask_MRI"],image_only=False),
            transforms.EnsureChannelFirstd(keys=["img_MRI", "mask_MRI"], channel_dim="no_channel"),
            # transforms.Orientationd(keys=["img_MRI"], axcodes="RAS"),
            # transforms.Spacingd(keys=["img_MRI","mask_MRI"], pixdim=(args.space_x, args.space_y, args.space_z), mode=("trilinear","nearest")),

            transforms.ScaleIntensityRanged(
                keys=["img_MRI"], a_min=args.a_min, a_max=args.a_max, b_min=args.b_min, b_max=args.b_max, clip=True
            ),
            transforms.CropForegroundd(keys=["img_MRI", "mask_MRI"], source_key="img_MRI", select_fn=lambda x: x > 0, margin=0), # 裁剪organ
            transforms.Resized(keys=["img_MRI"], spatial_size=(args.roi_x, args.roi_y, args.roi_z),
                               mode=("bilinear")),
            transforms.ToTensord(keys=["img_MRI", "mask_MRI"]),
        ]
    )
    nocrop_test_transform = transforms.Compose(
        [
            transforms.LoadImaged(keys=["img_MRI", "mask_MRI"],image_only=False),
            transforms.EnsureChannelFirstd(keys=["img_MRI", "mask_MRI"], channel_dim="no_channel"),
    ...
    if train_modality == 'crop':
        train_transform = crop_train_transform
        val_transform = crop_val_transform
        test_transform = crop_test_transform
    elif train_modality == 'nocrop':
        train_transform = nocrop_train_transform
        val_transform = nocrop_val_transform
        test_transform = nocrop_test_transform

...
class CropMultiPhaseLiverDataset(torch.utils.data.Dataset):
    def __init__(self, args, is_training=True):
        ...
        for item in anno:
            ...
            for phase in phase_list:
                img_name = item[0]+"_"+item[1]+"_"+phase
                # mp_img_list.append(f'{args.data_dir}/{item[0]}/{phase}.nii.gz')
                mp_img_list.append(f'{args.crop_data_dir}/{img_name}.nii.gz')
            if self.args.return_glb:
                mp_glb_img_list = []
                for phase in phase_list_glb:
                    mp_glb_img_list.append(f'{args.crop_data_dir}/{item[0]}/{phase}.nii.gz')
...
if __name__ == "__main__":
    ...
    dataset = CropMultiPhaseLiverDataset(args, is_training=True)

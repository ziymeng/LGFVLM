...
parser.add_argument("--json_list", default="mm_liver_data_fold_5.json", type=str, help="dataset json file")
...
parser.add_argument('--CLIP_text_pretrain_dir', default=f"./Text-emmbedding-gen/Liver_clip_txt_encoding.pth",
                    type=str)
parser.add_argument('--Bert_text_pretrain_dir', default=f"./Text-emmbedding-gen/Liver_bert_txt_encoding.pth",
                    type=str)
parser.add_argument("--logdir", default="VLM_liver_cls_7", type=str, help="directory to save the tensorboard logs")
...
def main():
    ...
    Liver_loader = get_loader(args,train_modality='nocrop')
    ...
    model.load_state_dict(torch.load(
        "checkpoints/Liver_data/cls/model.pt"), strict=False)
    model.cuda(args.gpu)
    ...
    inference_cls(model, crop_organ_loader, Liver_loader, args)
...
def inference_cls(model, crop_organ_loader, Liver_loader, args):
    ...
    ind = 0
    with torch.no_grad():
        for idx, (batch_crop, batch_liver) in enumerate(zip(crop_organ_loader, Liver_loader)):
            crop_data, crop_target, label = batch_crop["img_T2WI"], batch_crop["mask_T2WI"], batch_crop["label"]
            ...
            liver_data, liver_target = batch_liver["img_T2WI"], batch_liver["mask_T2WI"]
            crop_data, crop_target, liver_data, liver_target, labels = crop_data.cuda(args.rank), crop_target.cuda(
            args.rank), liver_data.cuda(args.rank), liver_target.cuda(args.rank), labels.cuda(args.rank)
            _, _, h, w, d = crop_target.shape

            with autocast(enabled=args.amp):
                val_preds, _ = model(crop_data, liver_data) 

            img_name = batch_crop["img_T2WI_meta_dict"]["filename_or_obj"][0].split("/")[-1]

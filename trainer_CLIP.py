...
def train_epoch(model, ct_loader, mri_loader, ct_unlabeled_loader, mri_unlabeled_loader, optimizer, scaler, epoch, loss_func1, loss_func2, args):
    model.train()
    ...
    for idx, (batch_ct, batch_mri, batch_ct_unlabeled, batch_mri_unlabeled) in enumerate(zip(ct_loader, mri_loader,ct_unlabeled_loader, mri_unlabeled_loader)):
        ...
        with autocast(enabled=args.amp):
            if epoch < args.start_fusion_epoch:
                ct_align_feat, ct_align_feat, ct_seg_out, mri_seg_out, ct_fusionfeat, mri_fusionfeat = model(ct_data, mri_data)
                ct_seg_loss = loss_func1(ct_seg_out, ct_target)
                ct_fusion_loss = loss_func1(ct_fusionfeat, ct_target)
                ct_total_loss = 0.7 * ct_seg_loss + 0.3 * ct_fusion_loss

                mri_seg_loss = loss_func1(mri_seg_out, mri_target)
                mri_fusion_loss = loss_func1(mri_fusionfeat, mri_target)
                mri_total_loss = 0.7 * mri_seg_loss + 0.3 * mri_fusion_loss
                loss = (ct_total_loss + mri_total_loss) / 2
            else:
                ct_align_feat, mri_align_feat, ct_seg_out, mri_seg_out, ct_fusionfeat, mri_fusionfeat = model(ct_data, mri_data)
                ct_seg_loss = loss_func1(ct_seg_out, ct_target)
                ct_fusion_loss = loss_func1(ct_fusionfeat, ct_target)
                ct_align_loss = loss_func2(ct_align_feat, ct_target)
                ct_total_loss = 0.6 * ct_seg_loss + 0.2 * ct_fusion_loss + 0.2 * ct_align_loss

                mri_seg_loss = loss_func1(mri_seg_out, mri_target)
                mri_fusion_loss = loss_func1(mri_fusionfeat, mri_target)
                mri_align_loss = loss_func2(mri_align_feat, mri_target)
                mri_total_loss = 0.6 * mri_seg_loss + 0.2 * mri_fusion_loss + 0.2 * mri_align_loss
                loss = (ct_total_loss + mri_total_loss) / 2
        if args.amp:
        ...
        if args.rank == 0:
            print(
                ...
                "ct_seg_loss: {:.4f}".format(ct_seg_loss),
                "ct_fusion_loss: {:.4f}".format(ct_fusion_loss),
                "ct_align_loss: {:.4f}".format(ct_align_loss),
                "mri_seg_loss: {:.4f}".format(mri_seg_loss),
                "mri_fusion_loss: {:.4f}".format(mri_fusion_loss),
                "mri_align_loss: {:.4f}".format(mri_align_loss),
        ...
        for idx, (batch_ct, batch_mri) in enumerate(zip(ct_loader,mri_loader)):
            ...
            with autocast(enabled=args.amp):
                if model_inferer is not None:
                    ct_align_feat, ct_align_feat, ct_seg_out, mri_seg_out, ct_fusionfeat, mri_fusionfeat = model_inferer(ct_data, mri_data)
                else:
                    ct_align_feat, ct_align_feat, ct_seg_out, mri_seg_out, ct_fusionfeat, mri_fusionfeat= model(ct_data, mri_data)
...
def run_training(
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        train_loss = train_epoch(
            model, ct_train_loader,mri_train_loader, ct_unlabeled_loader, mri_unlabeled_loader, optimizer, scaler=scaler, epoch=epoch, loss_func1=loss_func1, loss_func2=loss_func2, args=args
        )
        if args.rank == 0:
            print(

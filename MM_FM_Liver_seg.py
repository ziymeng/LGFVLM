...
from trainer_Liver_Seg_plus import run_training
from utils.MM_CLS_Liver_data_utils import get_loader
from monai.inferers import sliding_window_inference
from monai.losses import DiceCELoss
from monai.metrics import DiceMetric
from model.CLS.mm_classification_SwinUnter import MM_SwinUnter_Classification
from model.Seg.mm_Segmentation_Foundation_Model_plus import MM_Foundation_Model_Segmentation
from model.Seg.mm_Segmentation_SwinUnter_plus import MM_SwinUnter_Segmentation
...
parser.add_argument("--logdir", default="VLM_seg", type=str, help="directory to save the tensorboard logs")
parser.add_argument(
    "--pretrained_dir", default="./pretrained_models/", type=str, help="pretrained checkpoint directory"
)
parser.add_argument("--data_dir", default="./dataset/LLD_MMRI_data/", type=str, help="dataset directory")
parser.add_argument("--json_list", default="mm_liver_data.json", type=str, help="dataset json file")
...
parser.add_argument('--backbone', default='Foundation_model', choices=['Foundation_model','Semi_SM_model', 'SwinUNETR', 'VIT3D'], help='backbone [Foundation_model or SwinUNETR or VIT3D]')
parser.add_argument('--pretrain_dir', default=f"./runs/mm_TAO_pretrain_aug/model_params.pt", type=str)
parser.add_argument('--CLIP_text_pretrain_dir', default=f"./Text-emmbedding-gen/Liver_Bert_original_txt_encoding_Seg.pth", type=str)
parser.add_argument('--Bert_text_pretrain_dir', default=f"./Text-emmbedding-gen/Liver_Bert_original_txt_encoding_Seg.pth", type=str)
...
parser.add_argument('--fusion_module', default='Cross_Attention', choices=['DoubleAttention', 'Attention_Fusion', 'Multimoal_Cross_Attention', 'Cross_Attention', 'SingleAttention', 'Concat_Fusion'])
...
def main():
    ...
    args.logdir = "./out_TAO/" + args.logdir + "_" + args.text_prompt_name + "_" + args.fusion_module + "_" + use_text_prompt + "_" + res_depth
    if args.distributed:
...
def main_worker(gpu, args):
    ...
    ### load dataset
    t1_loader = get_loader(args,train_modality='T2WI')
    t1c_loader = get_loader(args,train_modality='T1IP')

    print(args.rank, " gpu", args.gpu)
    if args.rank == 0:
        print("Batch size is:", args.batch_size, "epochs", args.max_epochs)
    if args.backbone =="Foundation_model":
        model = MM_Foundation_Model_Segmentation(n_class=args.out_channels,
        ...
        model = MM_SwinUnter_Segmentation(n_class=args.out_channels,
                                                text_prompt=args.use_text_prompt,
                                                fusion_module=args.fusion_module,
                                                text_prompt_name=args.text_prompt_name,
                                                text_encoding=args.text_encoding_type,
                                                res_depth=args.res_depth)
    if args.pretrain:
        model.load_params(torch.load(args.pretrain_dir, map_location='cpu'))
    if args.text_prompt_name=="CLIP_embedding":
        ...
        print('load CLIP word embedding')
    if args.resume_ckpt:
        resume_ckpt_path = os.path.join(args.logdir,"model_final_v1.pt")
